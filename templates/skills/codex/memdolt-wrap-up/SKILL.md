...
6. Write only approved changed narratives through
   `memdolt state set --dir <repository> --actor "codex" --json` and
   `memdolt arch set --dir <repository> --actor "codex" --json`, supplying
...
locate, or repository-operation step. After it, the then-step 6 added only render;
