...
Notes:
- FULLTEXT costs on write are real (five internal index tables per FULLTEXT key **[V]**) — acceptable at memory-table row counts; a one-off, non-reproducible timing probe during M0 rig 3 (docs/spikes/m0-rig3.md §6, not part of the committed harness) observed ~1.7× per-insert overhead at a 200-row scale, consistent with but not a precise measurement of this note — the five-internal-table figure itself remains sourced to Dolt's own FULLTEXT blog post, §18, not independently reproduced (Dolt does not surface those tables to a SQL client). `MATCH ... AGAINST` in a `WHERE` clause also returns one row per matched index term, not one per matching document — a real fan-out property, not a write-cost one, that a naive per-source-type gather query must dedupe before applying `LIMIT` (docs/spikes/m0-rig3.md §4).
...
5. **Fusion:** weighted linear blend, memhub's exact formula and defaults — `relevance = 0.5·norm(lexical) + 0.5·cosine`, `score = relevance·age_decay − stale_penalty(0.3) − superseded_penalty(0.4)`, half-life off by default. Superseded facts are retained and gathered like any other row — §6.1's uniqueness is scoped to live rows precisely so they can be — and this penalty is what demotes them; recall never drops a hit for being superseded.
...
| R2 | Dolt FULLTEXT (tf-idf, NL-mode-only) drags Recall@K below baseline. **Golden gate run in M0 rig 3 [V]:** Recall@3 = 100% (21/21), 0 safety failures, matching the BM25 contingency's result on the same 22-query golden set — **but the gate did not discriminate: the ~21-row golden corpus exceeds `rerank_candidate_pool = 20` on every query, evicting one row each time, and which row differed by lexical config on 4 of 22 queries — it just was never a top-3 target, so a vector-only control (no lexical gather at all) also scored 100%. R2's actual lexical-quality question — does the choice matter once the excluded row is one a query wants — is not resolved; only re-tested at a scale too small to answer it.** | High | Golden gate in M0; in-process BM25 contingency (§8.1), built and validated but not activated on quality grounds — re-run at real (10³–10⁴-row) corpus size still needed. See `docs/spikes/m0-rig3.md` |
...
| MD5 | Dolt FULLTEXT first; in-process BM25 contingency on golden-gate failure | **Decided (M0 rig 3, docs/spikes/m0-rig3.md): Dolt FULLTEXT — on PRD-default + less-shipped-code grounds, not a recall-quality comparison. The golden gate passed (Recall@3 = 100%, 21/21, 0 safety failures) but could not discriminate FULLTEXT from BM25 or from no lexical gather at all at this corpus size (~21 rows vs. `rerank_candidate_pool = 20`); the contingency stays implemented and validated in reserve for a re-measurement at real corpus sizes** |
