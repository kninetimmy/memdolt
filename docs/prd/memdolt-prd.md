...
- Runtime: `yalue/onnxruntime_go` **[V]** (bundled shared libs for Windows AMD64, Linux ARM64, macOS ARM64; version-coupled to onnxruntime 1.26.0; CPU-only). **[verify]** Linux AMD64 bundling; supply `ONNXRUNTIME_SHARED_LIBRARY_PATH` path if absent.
- Tokenization is NOT bundled with the runtime — a Go WordPiece/HF-tokenizers implementation is required (candidate: `sugarme/tokenizer`; both models share the same BERT WordPiece vocab). **[verify]** — M0 spike must produce byte-identical token ids vs memhub's fastembed on a probe corpus.
