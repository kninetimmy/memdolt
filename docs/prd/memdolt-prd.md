...
Notes:
- FULLTEXT costs on write are real (five internal index tables per FULLTEXT key **[V]**) — acceptable at memory-table row counts; directionally measured in M0 rig 3 (docs/spikes/m0-rig3.md §6: ~1.7× per-insert cost at a 200-row scale; the five-internal-table figure itself is not independently reproduced — Dolt does not surface those tables to a SQL client — and remains sourced to Dolt's own FULLTEXT blog post, §18).
...
1. **Lexical gather:** per-source-type `MATCH … AGAINST (? IN NATURAL LANGUAGE MODE)`, limit 50/type. Dolt FULLTEXT is tf-idf-ish, not BM25, natural-language mode only — **measured in M0 rig 3 (docs/spikes/m0-rig3.md): the ranking behavior this induces cleared the golden gate at Recall@3 = 100%, matching the BM25 contingency's result on the same corpus** — see risk R2 and its contingency below.
...
**R2 contingency (lexical quality):** if the golden gate shows Dolt FULLTEXT dragging Recall@K below baseline, replace step 1 with in-process BM25 over candidate rows — memory corpora are small enough (10³–10⁴ rows) that full-scan lexical scoring in Go is trivially fast, and it removes the FULLTEXT write penalty as a bonus. Decide on measurement, not vibes. **[design]** — **M0 rig 3 measured both (docs/spikes/m0-rig3.md): Dolt FULLTEXT already clears the gate (Recall@3 = 100%, 21/21, 0 safety failures) on the golden set, so the contingency is built, validated, and held in reserve rather than activated — see MD5.**
...
| R2 | Dolt FULLTEXT (tf-idf, NL-mode-only) drags Recall@K below baseline. **Measured in M0 rig 3 [V]:** it doesn't — Recall@3 = 100% (21/21), 0 safety failures, matching the BM25 contingency's result on the same 22-query golden set | High | Golden gate in M0; in-process BM25 contingency (§8.1), built and validated but not activated. See `docs/spikes/m0-rig3.md` |
...
| MD5 | Dolt FULLTEXT first; in-process BM25 contingency on golden-gate failure | **Decided (M0 rig 3, docs/spikes/m0-rig3.md): Dolt FULLTEXT — the golden gate passed (Recall@3 = 100%, 21/21, 0 safety failures) without needing the contingency, which stays implemented and validated in reserve** |
