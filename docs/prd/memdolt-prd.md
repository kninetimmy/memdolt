...
| R2 | Dolt FULLTEXT (tf-idf, NL-mode-only) drags Recall@K below baseline. **Golden gate run in M0 rig 3 [V]:** Recall@3 = 100% (21/21), 0 safety failures, matching the BM25 contingency's result on the same 22-query golden set — **but the gate did not discriminate: a vector-only control with the lexical step removed entirely also scored 100%, because the ~21-row golden corpus never exceeds `rerank_candidate_pool = 20`. R2's actual lexical-quality question is not resolved; only re-tested at a scale too small to answer it.** | High | Golden gate in M0; in-process BM25 contingency (§8.1), built and validated but not activated on quality grounds — re-run at real (10³–10⁴-row) corpus size still needed. See `docs/spikes/m0-rig3.md` |
