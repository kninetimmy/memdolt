...
func TestVerifySessionValidatesBeforeInvocationAndRequiresExactInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nonobject data", raw: `{"data":[]}`},
		{name: "missing id", raw: `{"data":{"agent":"build"}}`},
		{name: "null id", raw: `{"data":{"id":null}}`},
...
func TestValidateSessionIDShape(t *testing.T) {
	for _, id := range []string{"ses_a", "sesABC-123", "ses0", "ses" + strings.Repeat("a", 252)} {
		if err := ValidateSessionID(id); err != nil {
			t.Errorf("valid id %q rejected: %v", id, err)
		}
	}
	for _, id := range []string{"", "ses", "session/1", "ses.dot", "ses space", "sesé", "ses&whoami", "ses%PATH%", "ses\n", "ses" + strings.Repeat("a", 253)} {
		if err := ValidateSessionID(id); err == nil {
